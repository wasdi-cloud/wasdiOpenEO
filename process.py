        ...
        for arg in args:
            if isinstance(arg, ParameterReference):
                if arg.from_parameter in named_parameters:
                    ...
                    wasdi.wasdiLog("ERROR Process Parameter " + arg.from_parameter + " was missing for process " + f.__name__)
        ...
        for k, arg in kwargs.items():
            if isinstance(arg, ParameterReference):
                if arg.from_parameter in named_parameters:
                    ...
                    wasdi.wasdiLog(
                        "ERROR Process Parameter " + arg.from_parameter + " was missing for process " + f.__name__)
            else:
...
standard_processes = [
    ...
    for _, func in inspect.getmembers(
        importlib.import_module("openEOWasdi"),
        inspect.isfunction,
    )
]